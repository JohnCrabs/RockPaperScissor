...
class WidgetImageViewer(QGLWidget):
    ...
    def paintGL(self):
        ...
        if self.isInputFromCamera:
            ...
            self.imgToView = cv2.cvtColor(frame, cv2.COLOR_BGR2RGBA)
            self.imgToView = cv2.rotate(self.imgToView, cv2.cv2.ROTATE_180)
...
def exec_app(w=512, h=512, minW=256, minH=256, maxW=512, maxH=512, winTitle='My Window', iconPath=''):
    ...
    widgetWin = WidgetImageViewer(w=w, h=h, minW=minW, minH=minH, maxW=maxW, maxH=maxH,
